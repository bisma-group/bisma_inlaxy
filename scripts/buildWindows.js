...
async function afterPackageBuilt (appPaths) {
  ...
  await appPaths.forEach(async function (packagePath) {
    var output = fs.createWriteStream(packagePath.replace('Inlaxy-', 'Inlaxy-v' + version + '-') + '.zip')
    var archive = archiver('zip', {
      zlib: { level: 9 }
    })
    archive.directory(packagePath, 'Inlaxy-v' + version)
    archive.pipe(output)
  ...
  if (appPaths.filter(p => p.includes('x64')).length > 0) {
    ...
    const options = {
      ...
      dest: 'dist/app/inlaxy-installer-x64',
    ...
    await installer(options)
      .then(function () {
        fs.renameSync('./dist/app/inlaxy-installer-x64/inlaxy-' + version + '-setup.exe', './dist/app/inlaxy-' + version + '-setup.exe')
      })
      .catch(err => {
